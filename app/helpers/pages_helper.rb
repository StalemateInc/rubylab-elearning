module PagesHelper
  def render_question(question, answers)
    ...
    when "textbox"
      out << text_field_tag("question-#{q_id}-answer", "", id: "q-#{q_id}-a-#{answers.id}")
    when "radio"
      answers.answers.each do |key, value|
        out << '<div class="test-answer-radio">'
        out << tag.span(value)
        out << radio_button_tag("question-#{q_id}-answer", key, id: "q-#{q_id}-a-#{answers.id}")
        out << '</div>'
      end
    when "checkbox"
      answers.answers.each do |key, value|
        ...
        out << tag.span(value)
        out << check_box_tag("question-#{q_id}", key)
        out << '</div>'
      end
    end
    out << '</div></div>'
